...
namespace Blocks.Gameplay.Core
{
    ...
    public class CoreHUD : NetworkBehaviour
    {
        ...
        private void ApplySceneVisibility()
        {
            ...
            bool showHud = IsGameplayScene(SceneManager.GetActiveScene().name);
            m_Root.style.display = showHud ? DisplayStyle.Flex : DisplayStyle.None;

            if (!showHud && m_MessageOverlay != null)
            {
                m_MessageOverlay.style.display = DisplayStyle.None;
            }
        }
