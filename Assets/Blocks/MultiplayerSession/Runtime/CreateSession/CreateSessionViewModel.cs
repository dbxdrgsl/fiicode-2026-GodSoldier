...
namespace Blocks.Sessions
{
    public class CreateSessionViewModel : IDisposable, IDataSourceViewHashProvider, INotifyBindablePropertyChanged
    {
        ...
        readonly string m_DefaultSessionName;
        ...
        public CreateSessionViewModel(string sessionType, string defaultSessionName = null)
        {
            m_DefaultSessionName = string.IsNullOrWhiteSpace(defaultSessionName)
                ? BuildFallbackSessionName(sessionType)
                : defaultSessionName;
            ...
            SessionName = m_DefaultSessionName;
        ...
        {
            sessionOptions.Name = string.IsNullOrWhiteSpace(SessionName)
                ? BuildFallbackSessionName(sessionOptions.Type)
                : SessionName;
            return await MultiplayerService.Instance.CreateSessionAsync(sessionOptions);
        }

        static string BuildFallbackSessionName(string sessionType)
        {
            var prefix = string.IsNullOrWhiteSpace(sessionType) ? "god-soldier" : sessionType.Trim();
